...
impl<'a> PdfPage<'a> {
    ...
    /// Renders a rectangular tile of this [PdfPage] into a newly allocated [PdfBitmap].
    ///
    /// `tile_left` and `tile_top` are offsets in the full rendered page's device-coordinate
    /// space selected by the given [PdfRenderConfig]. Only the tile-sized bitmap is allocated;
    /// no full-page bitmap is created. Callers should carry the effective full dimensions with
    /// their tile key and use [PdfRenderConfig::set_fixed_size()] to make them explicit.
    ///
    /// Rendering uses the same `FPDF_RenderPageBitmap` and `FPDF_FFLDraw` calls as full-page
    /// rendering, with a negative device origin. Pdfium therefore retains responsibility for
    /// page rotation, CropBox mapping, annotations, and form data.
    ///
    /// Callers that place adjacent tiles should request a small overlap around each tile and
    /// display only its core. Pdfium's antialiasing at a hard bitmap boundary can otherwise make
    /// the outermost pixels of objects crossing that boundary differ from a full-page render.
    ///
    /// Pdfium cannot combine form rendering with custom transformation matrices or clipping.
    /// Configurations that disable form-data rendering (including configurations changed by
    /// [PdfRenderConfig::clip()] or any transformation method) return
    /// [PdfiumError::PageRenderTileUnsupportedConfiguration].
    ///
    /// This call is synchronous and cannot be cancelled after Pdfium begins rendering. Callers
    /// should use bounded tile sizes and check cancellation between tile calls.
    pub fn render_tile_with_config(
        ...
        if !settings.do_render_form_data {
            return Err(PdfiumError::PageRenderTileUnsupportedConfiguration);
        }

        Self::validate_render_tile_bounds(&settings, tile_left, tile_top, tile_width, tile_height)?;
        ...
        self.render_tile_into_bitmap_with_settings(&mut bitmap, settings, tile_left, tile_top)?;
    ...
    fn render_tile_into_bitmap_with_settings(
        &self,
        bitmap: &mut PdfBitmap,
        settings: PdfPageRenderSettings,
        tile_left: Pixels,
        ...
        let tile_width = bitmap.width();
        let tile_height = bitmap.height();
    ...
    fn validate_render_tile_bounds(
        settings: &PdfPageRenderSettings,
        tile_left: Pixels,
        tile_top: Pixels,
        tile_width: Pixels,
        ...
        let tile_right = tile_left
            .checked_add(tile_width)
            .ok_or(PdfiumError::PageRenderTileOutOfBounds)?;
        let tile_bottom = tile_top
        ...
        if settings.width <= 0
            ...
            || tile_width <= 0
            || tile_height <= 0
            || tile_right > settings.width
            || tile_bottom > settings.height
        {
            return Err(PdfiumError::PageRenderTileOutOfBounds);
        }

        Ok(())
    }

    fn render_page_and_form_data_into_bitmap(
        &self,
        bitmap: &mut PdfBitmap,
        settings: &PdfPageRenderSettings,
        start_x: Pixels,
...
mod tests {
    ...
    #[test]
    fn test_tiled_rendering_matches_full_rendering() -> Result<(), PdfiumError> {
        let bindings = Pdfium::bind_to_library(Pdfium::pdfium_platform_library_name_at_path(
            "../pdfium/lib",
        ))?;
        let pdfium = Pdfium::new(bindings);
        let mut document = pdfium.load_pdf_from_file("./test/tile-render.pdf", None)?;

        // Add a live annotation so the FPDF_ANNOT path is exercised in both full and tile
        // rendering, rather than comparing page content alone.
        {
            let mut page = document.pages_mut().get(0)?;
            let mut annotation = page.annotations_mut().create_square_annotation()?;

            annotation.set_bounds(PdfRect::new_from_values(360.0, 110.0, 430.0, 250.0))?;
            annotation.set_fill_color(PdfColor::MAGENTA)?;
            annotation.set_stroke_color(PdfColor::NAVY)?;
        }

        for (page_index, page) in document.pages().iter().enumerate() {
            // Wider than the application's 1024px core so every fixture,
            // including /Rotate 90 and CropBox pages, crosses a real tile boundary.
            let config = PdfRenderConfig::new().set_target_width(1664);
            let full = page.render_with_config(&config)?;
            let full_width = full.width();
            let full_height = full.height();

            const BLEED: Pixels = 32;

            let core_left = full_width / 5;
            let core_top = full_height / 4;
            let core_width = 173.min(full_width - core_left - BLEED);
            let core_height = 191.min(full_height - core_top - BLEED);
            let tile_left = core_left - BLEED;
            let tile_top = core_top - BLEED;
            let tile_width = core_width + BLEED * 2;
            let tile_height = core_height + BLEED * 2;
            let tile = page.render_tile_with_config(
                &config,
                tile_left,
                tile_top,
                tile_width,
            ...
            let full_bytes = full.as_raw_bytes();
            let tile_bytes = tile.as_raw_bytes();

            assert_eq!((tile.width(), tile.height()), (tile_width, tile_height));

            let mut differing_channels = 0_usize;
            let mut total_delta = 0_usize;
            let mut maximum_delta = 0_u8;

            for row in 0..core_height as usize {
                let full_start =
                    ((core_top as usize + row) * full_width as usize + core_left as usize) * 4;
                let tile_start =
                    ((BLEED as usize + row) * tile_width as usize + BLEED as usize) * 4;
                let tile_row = &tile_bytes[tile_start..tile_start + core_width as usize * 4];
                let full_row = &full_bytes[full_start..full_start + core_width as usize * 4];

                for (tile_channel, full_channel) in tile_row.iter().zip(full_row) {
                    let delta = tile_channel.abs_diff(*full_channel);
            ...
            let channel_count = core_width as usize * core_height as usize * 4;

            eprintln!(
                "page {page_index}: {differing_channels}/{channel_count} channels differ; total delta {total_delta}; max {maximum_delta}"
            );
            // Integer device origins produce byte-identical output for pages rendered at an
            // integral scale. At fractional scales, Pdfium can choose slightly different edge
            // coverage for a small number of antialiased pixels even with a bleed gutter.
            assert!(differing_channels <= channel_count / 100);
            assert!(total_delta <= channel_count);

            // Compare a complete 1024px-core grid against the full-page render.
            // The earlier small-crop test could pass without ever exercising an
            // adjacent tile, masking rotated glyph culling at a bitmap edge.
            const GRID_CORE: Pixels = 1024;
            let mut grid_differing_channels = 0_usize;
            let mut grid_total_delta = 0_usize;
            let mut grid_maximum_delta = 0_u8;
            let mut grid_channel_count = 0_usize;
            let mut core_top = 0;
            while core_top < full_height {
                let core_height = GRID_CORE.min(full_height - core_top);
                let mut core_left = 0;
                while core_left < full_width {
                    let core_width = GRID_CORE.min(full_width - core_left);
                    let render_left = (core_left - BLEED).max(0);
                    let render_top = (core_top - BLEED).max(0);
                    let render_right = (core_left + core_width + BLEED).min(full_width);
                    let render_bottom = (core_top + core_height + BLEED).min(full_height);
                    let grid_tile = page.render_tile_with_config(
                        &config,
                        render_left,
                        render_top,
                    ...
                    let grid_bytes = grid_tile.as_raw_bytes();
                    let tile_core_left = (core_left - render_left) as usize;
                    let tile_core_top = (core_top - render_top) as usize;

                    for row in 0..core_height as usize {
                        let full_start = ((core_top as usize + row) * full_width as usize
                            + core_left as usize)
                            * 4;
                        let tile_start = ((tile_core_top + row) * grid_tile.width() as usize
                            + tile_core_left)
                            * 4;
                        let full_row =
                            &full_bytes[full_start..full_start + core_width as usize * 4];
                        let tile_row =
                            &grid_bytes[tile_start..tile_start + core_width as usize * 4];
                        for (tile_channel, full_channel) in tile_row.iter().zip(full_row) {
                            let delta = tile_channel.abs_diff(*full_channel);
                            grid_differing_channels += usize::from(delta != 0);
                            grid_total_delta += delta as usize;
                            grid_maximum_delta = grid_maximum_delta.max(delta);
                        }
                    }
                    grid_channel_count += core_width as usize * core_height as usize * 4;
                    core_left += GRID_CORE;
                }
                core_top += GRID_CORE;
            }
            ...
            eprintln!(
                "page {page_index} grid: {grid_differing_channels}/{grid_channel_count} channels differ; total delta {grid_total_delta}; max {grid_maximum_delta}"
            );
            assert!(grid_differing_channels <= grid_channel_count / 10_000 + 4);
            assert!(grid_total_delta <= grid_channel_count / 10_000 + 4);
            assert!(grid_maximum_delta <= 1);

            assert!(matches!(
                page.render_tile_with_config(&config, full_width - 1, full_height - 1, 2, 2,),
                Err(PdfiumError::PageRenderTileOutOfBounds)
            ));

            let clipped_config =
                PdfRenderConfig::new()
                    .set_target_width(612)
                    .clip(0, 0, full_width, full_height);

            assert!(matches!(
                page.render_tile_with_config(&clipped_config, 0, 0, 32, 32),
                Err(PdfiumError::PageRenderTileUnsupportedConfiguration)
            ));
        }

        let form_document = pdfium.load_pdf_from_file("./test/tile-form.pdf", None)?;
        let form = form_document
            .form()
            .expect("fixture must contain an AcroForm");

        assert_eq!(form.form_type(), PdfFormType::Acrobat);
        assert_eq!(
            form.field_values(form_document.pages()).get("tile_field"),
            Some(&Some("FORM VALUE".to_string()))
        );

        let page = form_document.pages().get(0)?;
        let form_config = PdfRenderConfig::new()
            .set_target_width(612)
            .highlight_text_form_fields(PdfColor::new(255, 0, 0, 96));
        let full_with_form = page.render_with_config(&form_config)?;
        let full_without_form = page.render_with_config(
            &PdfRenderConfig::new()
                .set_target_width(612)
                .render_form_data(false),
        )?;

        // The form-fill overlay (including its configured highlight) must contribute pixels
        // beyond the widget's static annotation appearance.
        assert_ne!(
            full_with_form.as_raw_bytes(),
            full_without_form.as_raw_bytes()
        );

        const FORM_BLEED: Pixels = 8;
        let core_left = 100;
        let core_top = 320;
        let core_width = 300;
        let core_height = 120;
        let tile_width = core_width + FORM_BLEED * 2;
        let tile = page.render_tile_with_config(
            &form_config,
            core_left - FORM_BLEED,
            core_top - FORM_BLEED,
            tile_width,
            core_height + FORM_BLEED * 2,
        )?;
        let full_bytes = full_with_form.as_raw_bytes();
        let tile_bytes = tile.as_raw_bytes();

        for row in 0..core_height as usize {
            let full_start = ((core_top as usize + row) * 612 + core_left as usize) * 4;
            let tile_start =
                ((FORM_BLEED as usize + row) * tile_width as usize + FORM_BLEED as usize) * 4;

            assert_eq!(
                &tile_bytes[tile_start..tile_start + core_width as usize * 4],
                &full_bytes[full_start..full_start + core_width as usize * 4]
            );
        }
