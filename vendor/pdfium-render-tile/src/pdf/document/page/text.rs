...
impl<'a> PdfPageText<'a> {
    ...
    /// Returns the character at the given zero-based page-text index without
    /// first allocating an index vector for every character on the page.
    ///
    /// This is useful for cancellable or bounded consumers that intentionally
    /// walk large text layers in small increments.
    #[inline]
    pub fn char_at(
        &self,
        index: PdfPageTextCharIndex,
    ) -> Result<PdfPageTextChar<'_>, PdfiumError> {
        if index >= self.len().max(0) as usize {
            return Err(PdfiumError::CharIndexOutOfBounds);
        }
        // SAFETY: the range check above establishes the accessor contract.
        Ok(unsafe { self.char_at_unchecked(index) })
    }

    /// Returns a character without querying PDFium for the page character
    /// count again.
    ///
    /// # Safety
    ///
    /// `index` must be less than this text page's current [`PdfPageText::len()`]
    /// and must fit in an `i32`. The text page must not have been closed.
    #[inline]
    pub unsafe fn char_at_unchecked(
        &self,
        index: PdfPageTextCharIndex,
    ) -> PdfPageTextChar<'_> {
        PdfPageTextChar::from_pdfium(
            self.page.document_handle(),
            self.page.page_handle(),
            self.text_page_handle(),
            index as i32,
