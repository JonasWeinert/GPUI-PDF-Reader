...
pub enum PdfiumError {
    ...
    /// A requested page render tile has a negative origin, lies outside the full rendered page,
    /// or has dimensions that overflow the full rendered page.
    PageRenderTileOutOfBounds,

    /// Tiled rendering currently requires a form-data-compatible [crate::pdf::document::page::render_config::PdfRenderConfig].
    /// Pdfium cannot combine form rendering with a custom transformation matrix or clipping
