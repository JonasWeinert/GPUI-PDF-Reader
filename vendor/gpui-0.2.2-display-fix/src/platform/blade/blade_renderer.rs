...
impl BladeRenderer {
    ...
    pub fn layer(&self) -> metal::MetalLayer {
        // `Surface::metal_layer()` returns a retained clone. Transfer that +1
        // ownership into metal's ForeignType wrapper; constructing the wrapper
        // directly from `layer_ptr()` would make its Drop release a borrowed
        // pointer and leave AppKit/Core Animation with a dangling layer.
        let layer = self.surface.metal_layer();
        let layer = objc2::rc::Retained::into_raw(layer) as *mut metal::CAMetalLayer;
        unsafe { foreign_types::ForeignType::from_ptr(layer) }
    }
