...
- Preserve explicit destination y coordinates through PDFium's view settings and page-to-device conversion, but treat them as a fallback when the outline title has an exact page-text match: authored coordinates are often page-level or stale while the visible heading is more useful. Lazily extract just that page, prefer the largest exact occurrence as the likely heading, then fall back to the explicit coordinate or page top.
- PDF link annotations expose page-space rectangles plus either a local destination or URI action. Convert all rectangle corners through the same rotation-aware page-to-device mapping as text; accept only bounded HTTP/HTTPS URIs.
- Internal link destinations are often only page-level or approximate. Extract the text under the source annotation, tolerate a neighboring edge glyph, normalize citation wrappers such as `[12]`, search near the authored destination, and expand numbered matches until the next reference marker. Keep the authored destination as the safe fallback.
- Center known outline coordinates and matched headings in the viewport, then clamp at the document edges. Page-top fallback remains top-aligned because centering an unknown position can expose the preceding page without adding useful context.
