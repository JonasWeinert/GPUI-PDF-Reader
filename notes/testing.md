...
- Theme coverage parses and applies every bundled config, checks required semantic tokens and icon assets, then launches native System, light, and dark cases and requires a settled exact-tile QA report. Inspect one light Fluid and one dark Classic window-only capture when changing token mappings.
