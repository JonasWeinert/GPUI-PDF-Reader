//! Bounded PDFium operations used by the thread-affine adapter.

use key_pdf_core::{
    PageSize, PdfLink as DocumentLink, PdfLinkTarget, PixelRect, TextBounds, TextChar, TileKey,
    TocEntry,
};
...
pub const MAX_RASTER_DIMENSION: u32 = 65_536;
pub const MAX_TILE_DIMENSION: u32 = 1_088;
pub const MAX_PAGE_TEXT_CHARS: usize = 100_000;
...
pub const MAX_TOC_TITLE_UTF16_BYTES: usize = 2_048;
...
pub(crate) fn page_sizes(document: &PdfDocument<'_>) -> Result<Vec<PageSize>, String> {
    document
        .pages()
        .page_sizes()
...
pub(crate) fn extract_table_of_contents(
    document: &PdfDocument<'_>,
    pages: &[PageSize],
) -> Vec<TocEntry> {
...
pub(crate) fn extract_document_links(
    document: &PdfDocument<'_>,
    pages: &[PageSize],
) -> Vec<DocumentLink> {
...
pub(crate) fn validated_link_url(uri: &str) -> Option<String> {
...
pub(crate) fn validate_tile_request(tile: TileRequest) -> Result<(), String> {
...
pub(crate) fn validate_text_character_count(count: i32) -> Result<usize, String> {
...
pub(crate) fn normalized_text_bounds(
    pixels: [(i32, i32); 4],
    pixel_width: u32,
    pixel_height: u32,
) -> Option<TextBounds> {
